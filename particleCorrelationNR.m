function [Pout,Funcval]=particleCorrelationNR(varargin)
	for i=1:nargin/2
	...
	% ub=P+0.2*abs(P);
	% tic
	Pout = particleswarm(@(P) getJac(P,dy,dx,x0,y0,F,4,choice2,subpos,save_name,coef_obj,func_ver),max(size(P)),lb,ub);
	% toc
	[G,Funcval]=getJac(Pout,dy,dx,x0,y0,F,1,choice2,subpos,save_name,coef_obj,func_ver);
	% Funcval
...
function varargout=getJac(P,dy,dx,X,Y,F,choice,choice2,subpos,save_name,coef_obj,func_ver)
