function [P_final,Corr_out,iterations]=NRtracking3(varargin)
	...
	while flag==0
		...
		if (search_flag==0)%&&(count>2)
			...
			Lo=xb-xa;
		   	for i=1:iterMax
		   		if i==1
		   			lambda1=xa+r^2*Lo;
		   			lambda2=xa+r*Lo;
		   			[G,f1]=getJac1_eff(coef,(P+(dP')*lambda1),dy,dx,x0,y0,F,1,subpos,stepsize,coef_shift,coef2,save_name);
		   			[G,f2]=getJac1_eff(coef,(P+(dP')*lambda2),dy,dx,x0,y0,F,1,subpos,stepsize,coef_shift,coef2,save_name);
			...
			% out=(xa+xb)/2
			[G,Funcval]=getJac1_eff(coef,(P+(dP')*(xa+xb)/2),dy,dx,x0,y0,F,1,subpos,stepsize,coef_shift,coef2,save_name);
			% Funcval
			% stepp=(dP')*(xa+xb)/2
		...
		elseif (search_flag==1)%||(count<3)
			...
			for j=1:6
				...
				[G,Funcval]=getJac1_eff(coef,P_check(j,:),dy,dx,x0,y0,F,1,subpos,stepsize,coef_shift,coef2,save_name);
