function [P_final,Corr_out]=DICtracking3(varargin)
	...
	dPfunc_dynamic=strcat(name_short,'_dPFunc');
	dPfunc_dynamic=str2func(dPfunc_dynamic);

	Warpfunc_dynamic=strcat(name_short,'_WarpFunc');
	Warpfunc_dynamic=str2func(Warpfunc_dynamic);

	Mat2Vec_dynamic=strcat(name_short,'_Mat2Vec');
	Mat2Vec_dynamic=str2func(Mat2Vec_dynamic);
	...
	while (flag==1)
		...
		% size(deltaP)
		W=updateWarp(P,deltaP,save_name);
...
function out=updateWarp(p,d,save_name)
	[~,name_short,~] = fileparts(save_name);
	WarpMat_dynamic=strcat(name_short,'_WarpMat');
	WarpMat_dynamic=str2func(WarpMat_dynamic);
	a=WarpMat_dynamic(p);
	b=WarpMat_dynamic(d');
	out=a*inv(b); % #4.23
end
