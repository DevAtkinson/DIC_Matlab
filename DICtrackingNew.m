function [P_final,Corr_out]=DICtrackingNew(F_in,G_in,subsize,subpos,gradx,grady,Pinitial)
	...
	for i=1:r
		for j=1:c
			...
			dfdx=gradx(i+subpos.coords(1)-1,j+subpos.coords(2)-1);
			dfdy=grady(i+subpos.coords(1)-1,j+subpos.coords(2)-1);
			%change
	...
	while (flag==1)
		...
		check=sqrt(deltaP(1)^2+(xpmax*deltaP(2))^2+(ypmax*deltaP(3))^2+deltaP(4)^2+(xpmax*deltaP(5))^2+(ypmax*deltaP(6))^2); % #4.24
