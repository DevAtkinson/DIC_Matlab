function [P_final,Corr_out,iterations]=NRtracking4(varargin)
	...
	while flag==0
		...
		% 	if  count<0 %(count<0)&&(mod(count,10)==0)%(count>1)&((norm(stepChange)<0.000001))&(dP_flag==0)
		% 		if (count==1)||(count==3)
		% 			dP=[1;0;0;0;0;0];
		% 			testing=testing+1;
		% 			dP_flag=1;
		...
		% 		[G,J,H]=getJac(coef,P,dy,dx,x0,y0,F,2,choice2,subpos,stepsize,coef_shift,save_name);
		...
		% 	else
		% 		[xa,xb]=GoldenBracket(coef,P,dP,dy,dx,x0,y0,F,1,subpos,stepsize,coef_shift,0.1,1,save_name);
		% 	end	

		% 	[G,Funcval,step_change,Pout]=goldenSection(coef,P,dP,dy,dx,x0,y0,F,choice2,subpos,stepsize,coef_shift,save_name,xa,xb);

		% 	if count>2
		% 		valid_check=Funcval/Funcval_store(count-1)<1.05;
		...
		% end
			...
			if  count<3
				for j=1:2
					...
					[xa,xb]=GoldenBracket(coef,P,dP,dy,dx,x0,y0,F,1,subpos,stepsize,coef_shift,0.1,2,save_name);
					[G,Funcval,step_change,Pout]=goldenSection(coef,P,dP,dy,dx,x0,y0,F,choice2,subpos,stepsize,coef_shift,save_name,xa,xb);
					Funcval_check(j)=Funcval;
					stepChange_check(j,:)=step_change;
					P_check(j,:)=Pout;
...
function [aA,aB]=GoldenBracket(coef,P,dP,dy,dx,X,Y,F,choice,subpos,stepsize,coef_shift,interval,choice2,save_name)
	% interval=0.05
	if choice2==2
		interval2=interval;
